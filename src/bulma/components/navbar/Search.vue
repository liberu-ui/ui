    ...
    >
      <fade v-if="isVisible">
        <div class="navbar-item search" v-click-outside="hide">
          <enso-typeahead v-bind="bindings" v-on="events" ref="typeahead">
            ...
            <template #option="{ highlight, item }">
              ...
              <span v-html="highlight(item['label'])" class="ml-2" />
              <span v-if="item.routes.length" class="route-controls">
                <span
                  v-for="itemRoute in item.routes"
                  :key="itemRoute.name"
      ...
      <a class="navbar-item" v-on="controlEvents" v-else>
        <fade>
